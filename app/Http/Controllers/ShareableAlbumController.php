...
class ShareableAlbumController extends Controller
{
    public function __invoke(
        ...
        if ($album === null || ! $this->albumHasPublicSongs($album)) {
        ...
        $info = $tab === 'information' ? $encyclopediaService->getAlbumInformation($album) : null;

        $siteName = (string) koel_branding('name');
        $description = $this->buildAlbumDescription($album->name, $album->artist->name, $siteName, $info?->wiki['summary'] ?? null);
        $image = $this->resolveImageUrl($album->cover)
            ?? $this->resolveImageUrl($info?->cover);

        $data['page_title'] = $album->name;
        $data['meta_description'] = Str::limit($description, 160);
        $data['og_title'] = $album->name;
        $data['og_description'] = Str::limit($description, 200);
        $data['og_image'] = $image;
        $data['og_url'] = $request->url();
        $data['og_type'] = 'website';
        $data['canonical_url'] = $request->url();
        $data['shareable_redirect'] = $this->buildShareableRedirect($album->id, $tab);
    ...
    private function albumHasPublicSongs(Album $album): bool
    {
        return $album->songs()->where('is_public', true)->exists();
    }

    private function buildAlbumDescription(string $name, string $artist, string $siteName, ?string $summary): string
    {
