<template>
  <div :class="config.sortable ? 'sortable' : 'unsortable'" class="song-list-header flex z-[2] bg-k-fg-3 pl-5">
    ...
    <span class="title-artist" data-testid="header-title" role="button" :title="t('ui.tooltips.sortByTitle')" @click="sort('title')">
      {{ t('songs.title') }}
      <template v-if="config.sortable">
    ...
    <template v-if="config.collaborative">
      <span
        ...
        role="button"
        :title="t('ui.tooltips.sortByUser')"
        @click="sort('collaboration.user.name')"
      >
        {{ t('users.roles.user.label') }}
        <template v-if="config.sortable">
          <Icon
      ...
      <span
        ...
        role="button"
        :title="t('ui.tooltips.sortByContributedAt')"
        @click="sort('collaboration.added_at')"
      >
        {{ t('ui.tooltips.contributed') }}
        <template v-if="config.sortable">
          <Icon
...
const albumColumnSortTooltip = computed(() => {
  if (props.contentType === 'episodes') {
    return t('ui.tooltips.sortByPodcast')
  }

  if (props.contentType === 'songs') {
    return t('ui.tooltips.sortByAlbum')
  }

  return t('ui.tooltips.sortByAlbumPodcast')
})
