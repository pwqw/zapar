...
const totalDuration = computed(() => secondsToHumanReadable(
  playableStore.state.playables.reduce((sum, p) => sum + (p.length ?? 0), 0),
  { hr: t('misc.hr'), min: t('misc.min'), sec: t('misc.sec') },
))
...
const playAll = async (shuffle: boolean) => {
  if (shuffle) {
    await queueStore.fetchRandom(500, true)
  } else {
    await queueStore.fetchInOrder(sortField, sortOrder, 500, true)
  }
