    ...
    <div v-if="useLastfm" data-testid="lastfm-integrated">
      ...
      <p>
        ...
        <a href="https://www.last.fm/about/trackmymusic" rel="noopener" target="_blank">{{
          $t('integrations.lastfm.scrobbling')
        }}</a
        >.
      </p>
      <div class="buttons mt-4 space-x-2">
        <Btn class="!bg-[var(--lastfm-color)]" @click.prevent="connect">{{
          connected ? $t('integrations.lastfm.reconnect') : $t('integrations.lastfm.connect')
        }}</Btn>
        <Btn v-if="connected" class="disconnect" gray @click.prevent="disconnect">{{
          $t('integrations.lastfm.disconnect')
        }}</Btn>
      </div>
    </div>
      ...
      <p>
        ...
        <span v-if="currentUserCan.manageSettings()" data-testid="lastfm-admin-instruction">
          ...
          <a href="https://docs.koel.dev/service-integrations#last-fm" target="_blank">{{
            $t('integrations.lastfm.documentation')
          }}</a>
